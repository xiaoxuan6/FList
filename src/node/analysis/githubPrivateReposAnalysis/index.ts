 ...
 * 代表一个私有github仓库
 * */
export interface GithubPrivateRepository {
    //github用户名
    user: string
    //仓库名
    repository: string,
...
export function githubPrivateReposAnalysis(config: GithubPrivateRepository): () => Promise<Folder> {
    return async (): Promise<Folder> => {
        async function getPath(path: string, dirName: string, hasDeep: number): Promise<Folder> {
            const headers = new Headers();
            headers.set("Accept", "application/vnd.github+json");
            if (typeof process.env.GITHUB_TOKEN === 'undefined' || process.env.GITHUB_TOKEN.trim() == '') {
                throw new Error("Github Token not empty");
            }
            ...
            for (const resJson of resJsons) {
                if (resJson.type == "file") {
                    ...
                    let pushThis = true;
                    if (resJson.name.toLocaleUpperCase() == "README.MD" || resJson.name.toLocaleUpperCase() == 'README.md') {
                        try {
        ...
        return await getPath(config.rootPath || "", "githubPrivateReposAnalysisRoot", config.maxDeep || 10);
    }
}